...
export function StoryView({ stories, onClose }: StoryViewProps) {
  ...
  useEffect(() => {
    if (isAutoPlay && !isTTSPlaying && !currentSlide?.audioUrl && (currentSlide?.caption || storyData?.title)) {
  ...
  }, [isTTSPlaying, isAutoPlay, currentSlide?.audioUrl, currentSlide?.caption, storyData?.title]);
      ...
      <div className="flex-1 flex items-center justify-center relative">
        ...
        <div className="absolute top-4 right-4 flex gap-2">
          ...
          {(currentSlide?.caption || storyData?.title) && (
            <button
              onClick={toggleTTS}
              className="p-3 bg-black/70 hover:bg-black/90 rounded-full transition-colors relative group"
              title={isTTSPlaying ? "Stop Whisper-style narration" : "Read text aloud with enhanced voice"}
            >
              {isTTSPlaying ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}
              {/* Quality indicator */}
